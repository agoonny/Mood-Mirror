...
export default function Home() {
  ...
  return (
    ...
    >
      ...
      <div className="glass shadow-neon absolute left-1/2 top-1/2 w-[min(92vw,720px)] -translate-x-1/2 -translate-y-1/2 rounded-3xl p-6">
        <div className="mb-4 flex items-center justify-center gap-3 text-xs uppercase tracking-[0.22em] text-slate-300/80">
          <span>Emotion: {profile.emotion}</span><span>•</span><span>Intensity: {profile.intensity.toFixed(2)}</span><span>•</span><span>Phase: {Math.min(3, Math.ceil(phase))}</span>
        </div>
        <input
          value={text}
